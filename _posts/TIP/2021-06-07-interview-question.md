...
- 자신만의 경쟁력
  : 도전의식이 저의 경쟁력이라고 생각합니다. 안될걸 아는데 도전합니다. 한번에 안되겠지만 여러번 도전하다보면 언젠간 될거라는 믿음이 있습니다. 이러한 마인드가 저의 경쟁력이라고 생각합니다. 
...
- 구독경제가 빅데이터에 미치는 영향 
  : 구독경제 즉 달마다 새로운 데이터가 생겨나고 이를 기반으로 미래 정책을 측정합니다. 그렇기 때문에 구독경제는 빅데이터의 시작점이라고 볼수있습니다. 또한 기업들이 구독경제를 모두 따라가는 경항은 이러한 구독 시스템, 무엇을 클릭했는지에 대한 시스템등 인터넷에서 수행하는 모든정보가 빅데이터를 구성하기 때문입니다. 
...
**디비와 웹**
  ...
  - 웹 어플리케이션 목적 : 인터넷을 이용해서 외부접근하는것이다. 
  - 예전에는 C/S프로그램이었다. 직접설치하고 내부 네트웍을 구성하는 방법이었다.
...
====
 ...
 - 인공지능 : 인간 대신 계산,인지,예측해주는 방법론
 - 머신러닝 : 인공지능의 한 분야로 인지,예측을 위한 다양한 알고리즘으로 연구됨
  
  머신러닝에는 인지,진단,패턴등을 분석하는 `inverse problom` 이 존재하고 제어,정책 학습 , 예측 등의 `Optimal control problem`이 존재합니다.

  그리고 미래의 개념인 뇌기반 인공지능(진짜 지능 - 분위기, 상황을 인지하고 스스로 판단하는) 미래 기술이다.
...
## Inverse problem
  ...
  - 지도학습 : 인버전의 문제이다. 
  - 비지도학습 : 비 지도 학습은 A의 값이 존재하지 않고 X의 값을 군집화하는 솔류션이다. 
    - 이를 통해서 y={X}에 의해서 값들의 분류를 설정할수있다. 라벨링 ~~ 

  - 데이터는 원인이 있어서 결과가 나온다.
  - 관측가능한 데이터로 부터 원래 무엇이었는지 알아내는게 inverseProblem이다.

  - 간단하게 말하면 블랙박스(함수) 이다. y = ax
  - 문제를 풀기위해 수학적인 기법이 사용되었다.

  - 즉 계산이다. 계산과정이 복잡하면 문제는 계산이 오래걸렸고 계산하기 힘들었다.
  - 그래서 이를 근사화 하기 시작하였고 신경망 같은 기법이 사용되었다.
  - 신경망 이론은 함수 근사화 기법이다. 이는 원래 계산신경과학 분야에서 먼저 발전 하였습니다. 


  - 관측 가능한 데이터에서 불확실성이 존재할수있고 확률모델로 신경망을 구성하기도 한다.
  - CNN은 입력에 특정 패턴을 알아내는 출력이고 신경망이 사용되었다고 합니다.

  - 텍스트 사이의 상관관계를 잘 추려 압축을 하고 추출된 키워드에서 토픽을 꺼냅니다.
  - 이러한 압축 방법은 이미지에서도 압축하여 노이지를 제거 할수있습니다. 
  
  - 이를 이용해서 멀티 모달로 인풋 포맷을 여러개로 가져갈수있습니다. specific하게 말이죠 


  **CNN**
  - 일반적인 신경망은 백터내적을 통해서 비선형 함수를 통과시키는게 대표적이다.
  - 하지만 씨엔엔는 필터링을 통한 컨볼류션한 오퍼레이션이 들어가고 비선형 함수를 통과한다. 
  - 그리고 풀링이 존재한다. 풀링(max)은 특정 패턴의 존재여부를 체크한다. 

  - 정밀하게 물체는 잡아내는것은 convolutions이 하고 있다고 말할수있다.
  - 노이즈에 흔들리지 않게 인식할 수 있도록 도움을 주는 부분은 풀링이라고 말할수있다. 

  - 이를 하나의 블럭으로 쌓고 조합을 하면 복잡한 패턴을 발견할수있고 이를 통해서 이미지 인식이 가능하다. 


  **AI의 이해**
  - 딥러닝 또는 학습과정을 사람이 이해하고 싶다. 
  - 어떻게 인지하는지 이를 시각화 하는 과정이 최근 화두였다. 


  **GAN**
  - 최근 딥러닝 영역의 화두이다.
  - 오버핏팅의 문제가 발생할때 <- (약간의 노이즈, 다른문제(다른데이터) 등을 추가하면 인식을 못하는 경우가 발생) 이를 해결하기 위한 방법으로 GAN이란 아이디어가 나옴
  - GAN은 딥러닝 알고리즘끼리 서로 경쟁을 하여 서로 성능을 이끌어내는 학습방법을 말함

## Optimal Control problem

  - 알파고를 상상하면 됩니다. 
  - 무엇가를 읽어내는것이 아니라 내가 원하는 반응을 이끌어내는 경우를 말한다.
  - 내 상대방의 환경이나 시스템( 타겟을) 원하는 상태로 이끌어내는 정책을 찾는것을 의미합니다.

  - 내가 과거에 했던 어떤 행동조합(원하는 상태로 이끄는것)으로 잘했는지 시간을 거슬러 올라가며 정보를 업데이트를 해야합니다.
  - 여러가지 제어기법중 하나로 강화학습으로 수렴이 되었습니다. 
  
  - 테스트 베드는 게임이다. 어떤 행동조합을 해야 승리를 할수있는지 결정하는 것이다. (시간을 거꾸러 전파하면서)


## 모델프리 컨트롤 &모델 베이스 컨트롤

  - 모델프리 컨트롤
    - 주어진 상황에서 그다음에 무엇을 할지 결정하는 정책
  - 모델베이스 컨트롤
    - 주어진 상황에서 상대방이 어떻게 할것인지 예측하고 배워나가는 정책

  - 모델베이스 컨트롤을 계층화하는 연구도 진행되고 있다. 


## 차세대 ai

  - 학습한 분야(특정분야)는 사람보다 더 잘하지만 사람이 더잘하는 게임(정책)이 많다.
    - 이러한 이유는 사람은 이전의 지식(학습한지식)을 활용하여 현재 사용하기 때문으로 볼수있다. 
    - 이러한 ai의 문제점을 해결하기위한 연구가 진행중이다. 
  
  - 사람, 사물에 대한 (융통성있는) 플렉서블한 목표를 설정할수없다. 
    - 목적이 조금만 벗어나면 해결할수없는 문제가 되어버린다.
    - 하지만 사람은 그렇지 않다.
    - 이를 해결해려면 목적을 다시설정하고 다시 학습시키고 처음부터 다시해야한다. 
    - 이러한 지능은 아직 할수없다. 



현재 기업및 국가는 DT(digitalTransfomation)을 추구하고 있습니다. 이는 무엇일까요?

인간의 사용자경험 즉 UX를 고려한 온오프라인의 디지털화 라고 말하고 싶습니다. 

온라인에서는 비대면, 가상 플랫폼을 생성하고 이를 UI 기반 UX로 소통의 장을 만드려고 디지털화 하는중입니다.

오프라인에서는 우리가 손으로 집적하는 모든행위를 기계가 스스로 하게끔 디지털전환중입니다. 

온오프라인에서 디지털화가 진행중이고 이는 인터넷 급의 변화를 또는 그보다 더 파격적인 변화를 예상 하고 있습니다. 

그렇다면 DT을 위해서 개발자인 내가 준비할 해야할 영역은 무엇일까요
 
DNA라고 불리는 영역입니다.

DT가 진행된 intelligent society에서 초연결이 진행될것입니다.

이는 빅데이터 처리, 빅데이터 분석 필요할것이고 빠른변화 수용을 위한 협업능력이 필요할것입니다.



클라우드로 전환하는 이유
 1. 온프레미스에서 클라우드 전환시 운영 비용감소 
 2. 확장을 위한 비즈니스 어질리티 (대응략) 
    - 서버 구매에 대한 빠른 대응
    - 트래픽증가에 대한 빠른 대응
...
빅데이터의 문제점 
 ...
 1. 데이터 정리가 너무 오래걸립니다.
 2. 지속적인 품질이 유지되지 않습니다. 
 3. 누구나 빅데이터 분석을 할수있었으면 좋겠다.
 4. 보안문제 ? 프라이빗 클라우드
 5. 데이터 마이그레이션 문제 ? 어떻게 진행될것인가. ? 
